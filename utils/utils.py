...
def read_dataset(root_dir, archive_name, dataset_name):
    ...
    if archive_name.lower() == 'mts_archive':
        file_name = os.path.join(root_dir, 'MTSArchive', f'{dataset_name}.csv')
        data = pd.read_csv(file_name, delimiter=',', header=0)  # 첫 번째 행을 헤더로 인식

        # 데이터 확인
        print(data.head())  # 데이터 확인용 출력

        # `activity`를 라벨로 지정하고, 센서 데이터 열만 피처로 선택
        y = data['activity']
        X = data[['acceleration_x', 'acceleration_y', 'acceleration_z', 'gyro_x', 'gyro_y', 'gyro_z']]  # 센서 데이터 열만 선택
        ...
        unique_classes_val, counts_val = np.unique(y_test, return_counts=True)
        print("Training Set Class Distribution:", dict(zip(unique_classes_train, counts_train)))
        print("Validation Set Class Distribution:", dict(zip(unique_classes_val, counts_val)))

        datasets_dict[dataset_name] = (x_train.copy(), y_train.copy(), x_test.copy(), y_test.copy())
    else:
        raise ValueError("현재 mts_archive 이외의 아카이브는 지원되지 않습니다.")
...
def read_all_datasets(root_dir, archive_name, split_val=False):
    ...
    if archive_name.lower() == 'mts_archive':
        for dataset_name in MTS_DATASET_NAMES:
            file_name = os.path.join(root_dir, 'MTSArchive', f'{dataset_name}.csv')
            print(f"Reading file: {file_name}")  # 파일 경로 확인용 출력
            data = pd.read_csv(file_name, delimiter=',', header=0)  # 첫 번째 행을 헤더로 인식
            print(data.head())  # 데이터 확인용 출력

            # `activity`를 라벨로 지정하고, 센서 데이터 열만 피처로 선택
            y = data['activity'].values  # .values로 numpy 배열로 변환
            ...
            print("Label distribution in y:", dict(zip(*np.unique(y, return_counts=True))))

            datasets_dict[dataset_name] = (x_train.copy(), y_train.copy(), x_test.copy(), y_test.copy())
    else:
        raise ValueError(f"지원되지 않는 아카이브입니다: {archive_name}. 현재 mts_archive만 지원됩니다.")
