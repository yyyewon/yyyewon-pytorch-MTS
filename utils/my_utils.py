...
def read_dataset(root_dir, archive_name, dataset_name):
    datasets_dict = {}
    # WalkvsRun.csv 파일을 학습 및 테스트 데이터로 분리
    if archive_name == 'mts_archive'or archive_name == 'WalvsRun_archive':
        file_name = root_dir + '/MTSArchive/' + dataset_name + '.csv'
        data = pd.read_csv(file_name, delimiter=',', header=None)
        y = data.iloc[:, 0]  # 첫 번째 열은 라벨
        X = data.iloc[:, 1:]  # 나머지 열은 피처

        # 데이터를 학습 및 테스트 데이터로 분할 (train_test_split 사용)
